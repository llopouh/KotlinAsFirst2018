...
fun averageStockPrice(stockPrices: List<Pair<String, Double>>): Map<String, Double> {
    ...
    val b = mutableMapOf<String, List<Double>>()
    for ((name, sum) in stockPrices)
        b[name] = b.getOrDefault(name, listOf()) + sum
    for ((name, price) in b) {
        val c = price.sum() / price.size
        a[name] = c
    }
    return a
}
...
fun findCheapestStuff(stuff: Map<String, Pair<String, Double>>, kind: String): String? =
        stuff.filter { (_, product) -> product.first == kind }.minBy { (_, product) -> product.second }?.component1()
...
fun whoAreInBoth(a: List<String>, b: List<String>): List<String> =
        a.filter { name -> b.contains(name) }.toSet().toList()
